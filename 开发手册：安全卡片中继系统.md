...
-   **EMQX:** 一款高性能的分布式MQTT消息队列遥测传输协议实现，用作本系统的实时消息代理。
...
-   **1.2. 核心组件:**
  * **客户端应用程序 (Flutter):** 单一代码库，根据用户选择运行于“传卡端”或“收卡端”模式。
    * **模块：** 用户界面 (UI/UX)、认证管理、角色管理、NFC交互 (物理卡读写/HCE模拟)、EMQX实时通信、本地安全存储、事件追踪与上报、端到端加密处理（若适用）。
  * **服务器端基础设施:**
    * **API服务 (Gin - Go语言):**
    ...
    * **EMQX Broker:** 实时消息队列，支持MQTT v5特性（如会话管理、用户属性、错误报告）。负责客户端之间状态同步消息和APDU数据的实时、安全传输。
    * **状态同步服务 (可作为API服务的一部分或独立微服务):** 在同一`userID`下配对的传卡端和收卡端之间，通过EMQX中继相关的实时状态更新。
...
-   **1.4. 安全原则:**
  ...
  * **访问控制:** EMQX主题访问权限采用最小权限原则 (ACLs)，基于`userID`和`clientID`进行细粒度控制。
...
-   **2.4. 客户端JWT与标识符存储:**
  ...
  * `user_id`和`client_id`也应被存储在App的安全内存或持久化存储中，供后续EMQX连接和API调用使用。
-   **2.5. 用户登出流程 (建议补充):**
...
-   **3.1. EMQX连接建立 (客户端 - Flutter):**
  1.  连接参数:
    * Broker地址: `mqtts://your-EMQX-broker.com:8883` (必须使用MQTTS，确保TLS证书有效)。
    * MQTT Client ID (协议级别): 使用登录时服务器下发的`clientID`。
    * Username: `clientID` (推荐) 或 `userID`。这取决于EMQX认证插件的配置。
  ...
  2.  EMQX Broker配置:
    * 必须配置认证插件 (例如，自定义插件或利用EMQX的可扩展性) 以：
      * 解析JWT。
      * 使用认证服务提供的公钥验证JWT签名。
...
-   **3.2. Redis数据结构定义 (角色与客户端状态):**
  ...
  * `client_state:{clientID}` (HASH): 存储每个活动`clientID`的详细实时状态。
    ...
    * `ip_address`: (STRING) MQTT连接源IP (EMQX提供给后端服务)。
...
-   **3.3. 角色选择与“挤下线”逻辑:**
  1.  客户端UI (Flutter): 成功登录并建立EMQX连接后，App显示角色选择界面。
  2.  客户端操作 (用户选择“传卡端”为例):
      a.  App发布MQTT消息：
      ...
      * Payload:
      ...
      ```
      * QoS: 1.
  3.  服务器 (角色与会话管理服务 - 订阅 `client/+/control/set_role_request` 或通过EMQX的规则引擎/钩子转发到API):
      a.  解析与验证: 从MQTT主题中提取`clientID`。从Payload中提取`userID`, `requested_role`, `request_id`。
      b.  身份验证: **关键安全步骤**: 验证Payload中的`userID`是否与当前MQTT连接（由`clientID`标识）认证的`userID`一致。EMQX的认证插件应能将此信息传递给后端。
...
-   **4.1. 同步原则:** 当任一端状态变化时，向服务器的指定EMQX主题发布“状态更新事件”。服务器的“状态同步服务”（可能集成在API服务中，或是一个独立的微服务监听EMQX）接收此事件，验证，更新Redis `client_state`，然后将处理/筛选后的事件版本转发给配对的另一端设备。
-   **4.2. 客户端状态更新事件发布主题 (Client -> Server via EMQX):** `client/{clientID}/event/state_update` (QoS: 1)
-   **4.3. 服务器状态同步服务订阅主题 (EMQX -> Server):** `client/+/event/state_update` (或配置EMQX规则引擎将此类消息推送到特定API端点)
-   **4.4. 对端状态同步下发主题 (Server -> Client via EMQX):** `client/{peer_clientID}/sync/peer_state_update` (QoS: 1)
...
-   **5.2. 审计日志数据库表: `user_activity_audit_log`**
  ...
  * `event_source_type`: (ENUM: "CLIENT_API_REQUEST", "CLIENT_MQTT_MESSAGE", "SERVER_INTERNAL_PROCESS", "EMQX_HOOK")
...
-   **6.1. 交易会话发起 (建议):**
  ...
  3.  **服务器处理:**
    ...
    * 响应客户端`transactionID`。
    * 通过EMQX向双方客户端下发交易开始指令，包含`transactionID`。
...
-   **7.1. 网络问题:**
  * **客户端 (Flutter):** 实现EMQX和HTTP API的自动重连（带指数退避和抖动策略）。对幂等操作进行安全重试。
  * **服务器 (EMQX LWT):** LWT机制处理意外断连，服务器相应更新`client_state`并通知对端。
  * **心跳机制:** MQTT `keepAliveInterval` 和应用层心跳（若有必要）来检测死连接。
-   **7.2. 数据校验:**
  * **服务器 (Gin):** 对所有客户端API参数、EMQX消息（若服务器解析其内容）进行严格的格式、类型、范围和逻辑校验。使用Gin的验证器或类似机制。
...
-   **7.5. 客户端奔溃恢复:** 客户端重启后，引导用户重新登录，重建EMQX连接，重新选择角色。服务器状态会相应更新。本地持久化应帮助恢复部分UI状态。
-   **7.6. 服务器故障与恢复 (高可用性):**
  * **EMQX集群:** 利用EMQX的集群能力保证消息服务高可用。
...
-   **7.7. 系统监控与告警 (建议补充):**
  * **指标监控:** 监控关键性能指标 (KPIs)，如API响应时间、错误率、EMQX消息吞吐量、队列深度、服务器资源使用率 (CPU, 内存, 网络)。使用Prometheus/Grafana等工具。
...
4.  **详细的部署和运维指南:** 针对EMQX集群、Gin服务集群、Redis Sentinel/Cluster、数据库高可用方案，补充相应的部署、监控、备份和故障演练指南。
5.  **安全性测试和渗透测试:** 定期进行安全代码审计和第三方渗透测试，确保系统安全性。
6.  **用户体验细节:**
  * 在网络不稳定或状态同步延迟时，客户端应有明确的加载或等待提示。
  * 错误提示应友好且易于理解，避免直接暴露技术细节。
  * “挤下线”通知应清晰告知用户原因和被哪台设备操作。
7.  **配置管理:** 所有可配置项（数据库连接串、JWT密钥、EMQX地址、第三方服务凭证等）应通过环境变量或配置文件管理，而不是硬编码在代码中。Go应用可以使用Viper等库管理配置。
